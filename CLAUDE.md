...
- El PDF está en la raíz del store como `recetario-digital-sustento.pdf`, 52 MB. `blobPathname` en `products.js` tiene que coincidir con ese nombre exacto: el store lo manda en el `content-disposition` y es el que ve el comprador al guardar el archivo. ⚠️ **Al resubirlo desde una Mac, no escribir el `.pdf` al renombrar:** macOS esconde las extensiones y el archivo termina como `.pdf.pdf`. Ya pasó una vez el 03/09/2026
