...
import { registrarCompra } from '../lib/registrar-compra.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Se firma con la compra ya confirmada, para que las 24hs del link
    // empiecen a correr cuando sale el mail y no antes.
    const link = await linkDeDescarga(product);
    ...
    await Promise.all([
      resend.emails.send({
        ...
        html: buildEmail({ buyerName, product, link }),
        text: textoDescarga({ buyerName, product, link })
      }),
      notificarVenta({
        product,
        buyerEmail,
